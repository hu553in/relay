...
function diagnostic(id: string, message: string): DiagnosticsEntry {
  return {
    id,
...
describe('useToastCenter', () => {
  it('baselines the first loaded diagnostics snapshot without replaying old entries', () => {
    const oldEntry = diagnostic('old', 'Already visible in diagnostics');
  ...
  it('toasts the first new diagnostic after an empty loaded snapshot', async () => {
    const newEntry = diagnostic('new', 'First live warning');
    ...
    expect(result.current.toasts[0]?.message).toBe('First live warning');
  });
});
